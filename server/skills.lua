...
local function get_skill(_src, skill_name)
    ...
    local query = string.format('SELECT skills FROM %s WHERE %s', config_sv.skills.sql.table_name, query_part)
...
local function get_all_skills(_src)
    ...
    local query = string.format('SELECT * FROM %s WHERE %s', config_sv.skills.sql.table_name, query_part)
    local response = MySQL.query.await(query, params)
...
local function insert_new_skill(_src, skill_name, skill_data)
    ...
    local query = string.format('INSERT IGNORE INTO %s (%s) VALUES (%s)', config_sv.skills.sql.table_name, table.concat(columns, ", "), values)
...
local function modify_skill(_src, skill_name, value, operation)
    ...
    if not all_skill_data[skill_name] then
        ...
        local default_skill_data = {
            level = config_sv.skills.skill_list[skill_name].level,
            current_xp = config_sv.skills.skill_list[skill_name].start_xp,
            required_xp = calculate_required_xp(config_sv.skills.skill_list[skill_name].level, config_sv.skills.skill_list[skill_name].first_level_xp, config_sv.skills.skill_list[skill_name].growth_factor),
            first_level_xp = config_sv.skills.skill_list[skill_name].first_level_xp,
            growth_factor = config_sv.skills.skill_list[skill_name].growth_factor,
            max_level = config_sv.skills.skill_list[skill_name].max_level
        }
    ...
    local query = string.format('UPDATE %s SET skills = ? WHERE %s', config_sv.skills.sql.table_name, query_part)
