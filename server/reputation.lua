...
local function get_reputation(_src, reputation_name)
    ...
    local query = string.format('SELECT reputation FROM %s WHERE %s', config_sv.reputation.sql.table_name, query_part)
...
local function get_all_reputations(_src)
    ...
    local query = string.format('SELECT * FROM %s WHERE %s', config_sv.reputation.sql.table_name, query_part)
    local response = MySQL.query.await(query, params)
...
local function insert_new_reputation(_src, reputation_name, reputation_data)
    ...
    local query = string.format('INSERT IGNORE INTO %s (%s) VALUES (%s)', config_sv.reputation.sql.table_name, table.concat(columns, ", "), values)
...
local function modify_reputation(_src, reputation_name, value, operation)
    ...
    if not all_reputation_data[reputation_name] then
        ...
        local default_reputation_data = {
            level = config_sv.reputation.reputation_list[reputation_name].level,
            current_rep = config_sv.reputation.reputation_list[reputation_name].start_rep,
            required_rep = calculate_required_rep(config_sv.reputation.reputation_list[reputation_name].level, config_sv.reputation.reputation_list[reputation_name].first_level_rep, config_sv.reputation.reputation_list[reputation_name].growth_factor),
            first_level_rep = config_sv.reputation.reputation_list[reputation_name].first_level_rep,
            growth_factor = config_sv.reputation.reputation_list[reputation_name].growth_factor,
            max_level = config_sv.reputation.reputation_list[reputation_name].max_level
        }
    ...
    local query = string.format('UPDATE %s SET reputation = ? WHERE %s', config_sv.reputation.sql.table_name, query_part)
