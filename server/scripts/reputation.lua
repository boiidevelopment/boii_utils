...
exports('reputation_calculate_required_rep', calculate_required_rep)
...
local function insert_new_reputation(_src, reputation_name, reputation_data)
    ...
    local query = string.format('INSERT IGNORE INTO %s (%s) VALUES (%s)', 'player_reputation', table.concat(columns, ', '), values)
...
local function get_reputation(_src, reputation_name)
    ...
    local query = string.format('SELECT reputation FROM %s WHERE %s', 'player_reputation', query_part)
    local response = MySQL.query.await(query, params)
    if response then
        debug_log('info', 'Fetched reputation data: ' .. response)
        for i = 1, #response do
            local row = response[i]
            local reputation_data = json.decode(row.reputation)
...
exports('reputation_get', get_reputation)
...
local function get_all_reputations(_src)
    ...
    local query = string.format('SELECT * FROM %s WHERE %s', 'player_reputation', query_part)
    local response = MySQL.query.await(query, params)
...
exports('reputation_get_all', get_all_reputations)
...
local function modify_reputation(_src, reputation_name, value, operation)
    ...
    if not all_reputation_data[reputation_name] then
        ...
        local default_reputation_data = {
            level = config.reputation[reputation_name].level,
            current_rep = config.reputation[reputation_name].start_rep,
            required_rep = calculate_required_rep(config.reputation[reputation_name].level, config.reputation[reputation_name].first_level_rep, config.reputation[reputation_name].growth_factor),
            first_level_rep = config.reputation[reputation_name].first_level_rep,
            growth_factor = config.reputation[reputation_name].growth_factor,
            max_level = config.reputation[reputation_name].max_level
        }
    ...
    local query = string.format('UPDATE %s SET reputation = ? WHERE %s', 'player_reputation', query_part)
...
exports('reputation_modify', modify_reputation)
